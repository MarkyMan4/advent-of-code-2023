...
func main() {
	...
	for scanner.Scan() {
		...
		lastNumsInSeqs := []int{readings[len(readings)-1]}
		prevDiffs := readings
		...
		for {
			...
			lastNumsInSeqs = append(lastNumsInSeqs, diffs[len(diffs)-1])
		}

		prediction := 0
		for i := len(lastNumsInSeqs) - 1; i >= 0; i-- {
			prediction = prediction + lastNumsInSeqs[i]
		}
